...
public:
    ...
    inline unsigned int GetLength()        {return _imgWidth * _imgHeight * _numChannel; }
    inline unsigned int GetImgWidth()      {return _imgWidth;}
    inline unsigned int GetImgHeight()     {return _imgHeight;}
	inline size_t GetReservedWidth()       {return _numBytes == 0 ? 0 : (_numBytes / (_imgHeight * _numChannel * sizeof(float))); }
    inline size_t GetDataSize()            {return _numBytes == 0? 0 : GetRequiredSize() ;}
    inline size_t GetRequiredSize()        {return sizeof(float)* _imgWidth * _imgHeight * _numChannel ;}
